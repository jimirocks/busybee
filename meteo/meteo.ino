...
unsigned long lastTimePulse = 0;          // For storing the last time state changed
...
void loop() {    
  ...
  if (currentMillis - lastTimePulse >= 1000) {
    ...
    if (currentMillis - lastTime >= 10000) {
      ...
      int windSpeedInt = round(maxWindSpeed);
      maxWindSpeed = 0; // reset max
      // sanitize overflow
      if (windSpeedInt > 63) {
        windSpeedInt = 63;
      }
    ...
    lastTimePulse = currentMillis;        // Time update  
  }
}